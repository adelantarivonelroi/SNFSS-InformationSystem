  ...
  if (isset($_REQUEST['id']))
  {
    ...
    if (ctype_digit($_REQUEST['id']))
    {
      ...
      while ( $row = mysqli_fetch_array($result_annoucements))
      {
        ...
        $sql_getname = "SELECT u.FirstName, u.LastName FROM post p INNER JOIN user u ON p.UserID = u.UserID WHERE u.UserID = $puid";
      ...
      if( isset($_POST['update'])) {
          ...
          $con->query($sql_update) or die(mysqli_error($con));
           header('location: viewdetailenrollmentreq.php');
        }
    }
    else
    {
